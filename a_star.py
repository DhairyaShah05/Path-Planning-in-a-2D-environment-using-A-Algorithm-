...
def create_shape_map(width, height, buffer):
    ...
    for y_coord in range(0, height):
        for x_coord in range(0, width):
            rect_1_1_temp = (x_coord + buffer) - (100)
            rect_1_2_temp = (y_coord + buffer) - (100)
            rect_1_3_temp = (x_coord - buffer) - (175)
            rect__4_temp = (y_coord - buffer) - (500)

            recta_2_1_temp = (x_coord + buffer) - (275)
            recta_2_2_temp = (y_coord + buffer) - 0
            recta_2_3_temp = (x_coord - buffer) - (350)
            recta_2_4_temp = (y_coord - buffer) - (400)

            hexagon_size_6_temp = (y_coord + buffer) +  0.58*(x_coord + buffer) - (475.098)
            hexagon_size_5_temp = (y_coord + buffer) - 0.58*(x_coord - buffer) + (275.002)
            hexagon_size_4_temp = (x_coord - buffer) - (779.9)
            hexagon_size_3_temp = (y_coord - buffer) + 0.58*(x_coord - buffer) - (775.002)
            hexagon_size_2_temp = (y_coord - buffer) - 0.58*(x_coord + buffer) - (24.92)
            hexagon_size_1_temp = (x_coord + buffer) - (520.1)

            trap_a_temp = (x_coord + buffer) - (900)
            trap_b_temp = (x_coord + buffer) - (1020)
            trap_c_temp = (x_coord - buffer) - (1100)
            trap_d_temp = (y_coord + buffer) - (50)
            trap_e_temp = (y_coord - buffer) - (125)
            trap_f_temp = (y_coord + buffer) - (375)
            trap_g_temp = (y_coord - buffer) - (450)
            
            w1_temp = (y_coord) - (buffer)
            w2_temp = (y_coord) - (500-buffer)
            w3_temp = (x_coord) - (buffer)
            w4_temp = (x_coord) - ((1200-buffer))
